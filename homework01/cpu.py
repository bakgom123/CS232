...
class CPU(threading.Thread):
    def __init__(self, ram, os, startAddr, debug, batch = False, num=0):
        ...
        ###CODE ADDED###
        self._mode = batch # set it to False
    ...
    def run(self):
        ...
        if self._mode == True: # If it's in batch mode, then it repeatedly calls _run_program() on each program in the list
            while (self._ram[self._startaddr] != 0): # These programs will be terminated with the location value 0
                self.set_pc(self._ram[self._startaddr])
                self._registers['reg0'] = self._registers['reg1'] = self._registers['reg2'] = 0 # clear the registers
                self._run_program()
                self._startaddr += 1 # move the address to the next one
        else: # Otherwise, it will just run the program normally (not in batch mode)
            self._run_program()
