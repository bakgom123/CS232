...
@date 02/03/2022
I have added the instruction and function to run the programs whose starting locations are in the array starting at location addr
'''
...
class Monitor:
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    print("R <addr>: run the programs whose starting locations are in the array starting at location addr")
    ...
    def _run_batch(self, addr):
        ...
        self._cpu = CPU(self._ram, calos.CalOS(), addr, self._debug, True) # since it is set to False in the constructor, change the value to True to let it start the program in batch
        self._cpu.start()		# call run()
        self._cpu.join()		# wait for it to end
